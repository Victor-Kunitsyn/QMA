...
COpr::COpr(DWORD callingThreadID)
{
	m_hThread = NULL;
	m_callingThreadID = callingThreadID;
...
short COpr::Connect(DWORD dwIP, unsigned short usPort, const CString& strUser, const CString& strPwd)
{
	if (m_hThread)
	{
		DWORD dwExitCode;
...
LRESULT COpr::OnConnectResult(short sResult,short sPri)
{
	::PostThreadMessage(m_callingThreadID, WM_MONITOR_CONNECT_RESULT, sResult, sPri);
    return 0;
}

LRESULT COpr::OnDisconnected(short sReason)
{
	::PostThreadMessage(m_callingThreadID, WM_MONITOR_DISCONNECTED, sReason, 0);
    return 0;
}

LRESULT COpr::OnPlayVideoResult(short sResult)
{
	InterlockedExchange(&g_lImagesWaitShow,0);
	::PostThreadMessage(m_callingThreadID, WM_PLAYVIDEO_RESULT, sResult, 0);
    return 0;
}



LRESULT COpr::OnVideoStopped(short sReason)
{
	::PostThreadMessage(m_callingThreadID, WM_VIDEO_STOPPED, sReason, 0);
    return 0;
}

LRESULT COpr::OnPlayAudioResult(short sResult)
{
	::PostThreadMessage(m_callingThreadID, WM_PLAYAUDIO_RESULT, sResult, 0);
    return 0;
}

LRESULT COpr::OnAudioStopped(short sReason)
{
	::PostThreadMessage(m_callingThreadID, WM_AUDIO_STOPPED, sReason, 0);
    return 0;
}

void COpr::OnStartTalkResult(short sResult)
{
	::PostThreadMessage(m_callingThreadID, WM_STARTTALK_RESULT, sResult, 0);
}

void COpr::OnTalkStopped(short sReason)
{
	::PostThreadMessage(m_callingThreadID, WM_TALK_STOPPED, sReason, 0);
}

void COpr::OnMonitorParamsChanged(unsigned short usCode,unsigned short usValue)
{
	::PostThreadMessage(m_callingThreadID, WM_MONITOR_PARAMS_CHANGED, usCode, usValue);	
}

void COpr::OnAlarm2(short sAlarm,short sMotionLeft,short sMotionTop,short sMotionRight,short sMotionBottom)
{
	::PostThreadMessage(m_callingThreadID, WM_ALARM2,sAlarm, MAKELPARAM(MAKEWORD(sMotionLeft,sMotionTop), MAKEWORD(sMotionRight,sMotionBottom)));
}

void COpr::OnStatistic(long lVideoBandwidth,short sVideoFrames,long lAudioBandwidth,short sAudioSamples)
{
	::PostThreadMessage(m_callingThreadID, WM_STATISTIC, lVideoBandwidth, MAKELPARAM(lAudioBandwidth,MAKEWORD(sVideoFrames,sAudioSamples)));	
}

LRESULT COpr::OnOtherDevicesParamsChanged(OTHER_DEVICE_PARAMS * pOtherDevicesParams)
{
	if (! ::PostThreadMessage(m_callingThreadID, WM_OTHERDEVICES_PARAMS_CHANGED, (WPARAM)pOtherDevicesParams, 0))
		delete pOtherDevicesParams;
    return 0;
}

LRESULT COpr::OnImage(IMAGE * pImage,bool bPlay)
{
	if (::PostThreadMessage(m_callingThreadID, WM_VIDEO, (WPARAM)pImage, (LPARAM)bPlay))
...
LRESULT COpr::OnAudio(AUDIO * pAudio,bool bPlay)
{
	if (!::PostThreadMessage(m_callingThreadID, WM_AUDIO, (WPARAM)pAudio, (LPARAM)bPlay))
	{
		delete [] pAudio->pData;
		delete pAudio;
	}
...
DWORD COpr::ThreadProc(LPVOID lpParameter)
{
	...
	p->bConnected = false;
	bool bVideoTryPlaying = false,bVideoPlaying = false,bAudioTryPlaying = false,bAudioPlaying = false,bAVLogon = false;
...
#define TALK_BUFFER_SIZE		2500
	...
	while (1)
	{
		::Sleep(CAMERA_SLEEP_TIME); //just to give a CPU cycles to another threads
